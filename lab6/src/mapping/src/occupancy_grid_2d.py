...
class OccupancyGrid2d(object):
    ...
    def LoadParameters(self):
        ...
        if not rospy.has_param("~topics/sensor"):
            return False
        self._sensor_topic = (
            rospy.get_param("~topics/sensor"))
        
        if not rospy.has_param("~topics/vis"):
            return False
        self._vis_topic = (
            rospy.get_param("~topics/vis"))
    ...
    def SensorCallback(self, msg):
        ...
        for idx, r in enumerate(msg.ranges):
            ...
            # Get angle of this ray in fixed frame. TODO
            ang = yaw + msg.angle_min + idx * msg.angle_increment
            ...
            # TODO!
            walk_inc = min(self._x_res, self._y_res)
            # updated = set()
            grid_x, grid_y = self.PointToVoxel(sensor_x, sensor_y)
            for a in np.arange(0, r, walk_inc):
                grid_x, grid_y = self.PointToVoxel(sensor_x + a * np.cos(ang), sensor_y + a * np.sin(ang))
                # if (grid_x, grid_y) in updated:
                #     continue
                self._map[grid_x][grid_y] += self._free_update
                self._map[grid_x][grid_y] = max(self._map[grid_x][grid_y], self._free_threshold)
            self._map[grid_x][grid_y] += -self._free_update + self._occupied_update
            self._map[grid_x][grid_y] = min(self._map[grid_x][grid_y], self._occupied_threshold)
