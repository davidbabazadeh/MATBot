...
def controller(waypoint):
  """
  waypoint is a list of waypoints
  """
  """
  ...
  Kp = np.diag([2.0, 0.8]) # TODO: You may need to tune these values for your turtlebot
  ...
  while not rospy.is_shutdown():
    try:
      ...
      waypoint_trans.pose.position.x = float(waypoint[0]) # TODO: what value would you use here?
      waypoint_trans.pose.position.y = float(waypoint[1]) # TODO: what value would you use here?
      waypoint_trans.pose.position.z = 0.0 # TODO: what value would you use here?  # Assuming the waypoint is on the ground

      quat = quaternion_from_euler(0.0, 0.0, float(waypoint[2])) # TODO: what would be the inputs to this function (there are 3)
      ...
      z_error = yaw
      error = np.array([-x_error, z_error]) # TODO: what are two values that we can use for this np.array, and what are the dimensions
      ...
      print(error, integral.shape, proportional.shape, derivative.shape)
      msg = Twist()
      ...
      if np.abs(error[0]) < 2: #TODO: what is our stopping condition/how do we know to go to the next waypoint?
...
def planning_callback(msg):
  try:
    ...
    print("131")
    for waypoint in trajectory: # TODO: write a loop to loop over our waypoints and call the controller function on each waypoint
      print("nnnnnext waypoint")
      controller(waypoint)
